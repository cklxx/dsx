...
export function useApp() {
  ...
  async function switchThread(threadId) {
    ...
    try {
      ...
      // Load history
      const read = await rpc.send('thread/read', { threadId, includeTurns: true })
      messages.value = []
    ...
    for (const item of items) {
      ...
      if (t === 'userMessage') {
        const text = (item.content || [])
          ...
          .join('\n')
        if (text) {
          messages.value.push({ id: item.id, kind: 'userMessage', role: 'user', text })
        }
      } else if (t === 'agentMessage' && item.text) {
        messages.value.push({
          id: item.id, kind: 'agentMessage', role: 'assistant',
          text: item.text, status: 'done',
        })
      } else if (t === 'reasoning') {
        const parts = []
        ...
        if (text) {
          messages.value.push({
            id: item.id, kind: 'reasoning', role: 'assistant',
            text, status: 'done',
          })
        }
      } else if (t === 'commandExecution') {
        messages.value.push({
          id: item.id, kind: 'commandExecution', role: 'assistant',
          title: item.command || '执行命令',
          command: item.command,
          exitCode: item.exitCode,
          output: item.aggregatedOutput || '',
          durationMs: item.durationMs,
          status: item.exitCode === 0 ? 'done' : (item.exitCode != null ? 'failed' : 'done'),
        })
      } else if (t === 'fileChange') {
        const ok = !item.status || item.status === 'completed' || item.status === 'applied'
        messages.value.push({
          id: item.id, kind: 'fileChange', role: 'assistant',
        ...
        messages.value.push({
          id: item.id, kind: 'webSearch', role: 'assistant',
          title: item.query || '搜索',
          query: item.query,
          status: 'done',
        })
        ...
        messages.value.push({
          id: item.id, kind: 'mcpToolCall', role: 'assistant',
        ...
        messages.value.push({
          id: item.id, kind: 'plan', role: 'assistant',
          title: '规划',
          text: item.text || '',
          status: 'done',
        })
  ...
  function handleItemStarted(item) {
    ...
    if (toolMeta) {
      messages.value.push({
        id: item.id, kind: t, role: 'assistant',
        ...toolMeta,
        status: 'running',
      })
      return
    ...
    if (t === 'contextCompaction') {
      messages.value.push({
        id: item.id || `sys-${Date.now()}`, kind: 'system', role: 'system',
        text: '📦 上下文已压缩',
      })
    }
  }
  ...
  function handleItemCompleted(item) {
    ...
    if (t === 'agentMessage') {
      ...
      if (item.command) {
        msg.title = item.command + (item.durationMs ? `  (${(item.durationMs / 1000).toFixed(1)}s)` : '')
  ...
  function toolMetaFor(item) {
    const t = item.type
    if (t === 'commandExecution') return {
      title: item.command || '执行命令',
      command: item.command,
      output: '',
      exitCode: null,
    }
    if (t === 'fileChange') return {
      title: `${(item.changes?.length || 0)} 个文件`,
      changes: item.changes || [],
    }
    if (t === 'webSearch') return {
      title: item.query || '搜索',
      query: item.query,
    }
    if (t === 'mcpToolCall') return {
      title: `${item.server ? item.server + '/' : ''}${item.tool || ''}`,
      tool: item.tool, server: item.server,
      arguments: item.arguments,
    }
    if (t === 'dynamicToolCall') return {
      title: `${item.namespace ? item.namespace + '/' : ''}${item.tool || ''}`,
      tool: item.tool, namespace: item.namespace,
      arguments: item.arguments,
    }
    if (t === 'plan') return {
      title: '规划',
