...
#[command(
    version,
    override_usage = "dsx exec [OPTIONS] [PROMPT]\n       dsx exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running DSX outside a Git repository.
    ...
    /// Do not load `$DSX_HOME/config.toml`; auth still uses `DSX_HOME`.
