            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun dsx doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("DSX_HOME: {}", config.codex_home.display()));
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run dsx login again or provide a supported auth env var.");
            }
            check
        }
        ...
        .details(details)
        .remediation("Run dsx login or provide an API key through a supported auth env var."),
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "DSX_HOME", &config.codex_home);
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match codex_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "DSX_HOME was resolved without config",
        )
        .detail(format!("DSX_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "DSX_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
