...
impl EnvironmentManager {
    ...
    /// Builds a manager from `DSX_HOME` and local runtime paths used when
    /// creating local filesystem helpers.
    ///
    /// If `DSX_HOME/environments.toml` is present, it defines the configured
    ...
    /// reading user config files from `DSX_HOME`.
