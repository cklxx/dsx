...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.dsx/config.toml` snippet:
    ///
    /// ```toml
    ...
    /// file: DSX_HOME/.credentials.json
    ...
    /// Directory containing all Codex state (defaults to `~/.dsx` but can be
    /// overridden by the `DSX_HOME` environment variable).
    pub codex_home: AbsolutePathBuf,

    /// Directory where Codex stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Codex writes log files (defaults to `$DSX_HOME/log`).
    pub log_dir: PathBuf,
...
/// Patch `DSX_HOME/config.toml` project state to set trust level.
/// Use with caution.
