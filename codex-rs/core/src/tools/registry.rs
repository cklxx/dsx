...
pub struct ToolRegistry {
    ...
    /// When the wire cannot carry namespaces, model calls arrive as plain flat
    /// names (`mcp__server__tool`). Map those back to the logical runtime.
...
impl ToolRegistry {
    ...
    fn add_flat_tool_aliases(&mut self) {
        for (name, tool) in &self.tools {
            ...
            {
                // Collision: leave the alias unregistered so the model cannot
                // ambiguously target two logical tools. Planner should have
                // failed earlier for reachable collisions.
    ...
    pub(crate) async fn dispatch_any_with_terminal_outcome(
        ...
        let tool = match self.resolved_tool(&tool_name) {
            Some((resolved_tool_name, tool)) => {
                // Resolve flat wire names (Anthropic) back to the logical ToolName
                // so handlers/hooks see the namespaced identity.
