...
Set a stable path to the skill CLI (default `DSX_HOME` is `~/.dsx`):

```
export DSX_HOME="${DSX_HOME:-$HOME/.codex}"
export IMAGE_GEN="$DSX_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$DSX_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
