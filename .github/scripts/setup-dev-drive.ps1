...
if (Test-Path "D:\") {
    ...
    try {
        $VhdPath = Join-Path $env:RUNNER_TEMP "dsx-dev-drive.vhdx"
        $SizeBytes = 64GB
        ...
        $Volume = $Partition | Format-Volume -FileSystem ReFS -NewFileSystemLabel "dsxDevDrive" -DevDrive -Confirm:$false -Force -ErrorAction Stop
